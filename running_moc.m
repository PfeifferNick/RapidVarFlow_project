function[H,V,H_normal,H_transient,V_normal,V_transient] = running_moc(friction,tsteps,theta,K_ut,K_ux,H0,V_initial,nsT,maxit,AsT,tol,Cd,Av)
...
%     end 
    ...
    if (1-t/Tcl) >= 0 
    V(n+1,j+1) = phi*T_closure*sqrt(2*g*H(n+1,j));
    else 
    V(n+1,j+1) = 0;
    end
